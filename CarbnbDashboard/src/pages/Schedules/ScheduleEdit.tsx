...
const ScheduleCreate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${BASE_URL}/member/schedules/show/${id}`, {
          method: 'POST',
          credentials: 'include',
          headers: {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            minimumHour: data.minimumHour != null ? data.minimumHour : '1',
            charger: data.charger != null ? data.charger.toString() : '',
          ...
          setSelectedSpotOption(data.spot.toString());
          setIsSpotOptionSelected(true);

          if (data.charger != null && data.charger == 1) {
            setSelectedChargerOption(data.charger.toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/member/schedules/update/${id}`, {
        method: "PUT",
        credentials: 'include',
        body: formDataToSend,
      });
