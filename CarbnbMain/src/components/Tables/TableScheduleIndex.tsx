...
const TableScheduleIndex = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${BASE_URL}/member/schedules/index`, {
          method: 'POST',
          credentials: 'include',
          headers: {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/member/schedules/remove/${id}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default">
      <div className="border-b-2 py-5 px-4 sm:pl-6 lg:pl-10 lg:pr-6">
        <div className="flex justify-between items-center gap-2">
          <h4 className="text-lg font-bold text-black">List of Schedules</h4>
        ...
        <div>
          <div className="max-w-full overflow-x-auto">
            <table className="w-full table-auto table-border">
              <tbody>
              {currentSchedules.map((schedule, key) => (
                <tr key={key} className="hover:bg-gray-100">
                  <td className="border-b py-5 px-4 sm:pl-6 lg:pl-10">
                    <NavLink to={`${schedule.id}`} className="flex gap-5">
                      {schedule.imagePath ? (
                      <img
                        className="schedule-image"
                        src={`${BASE_URL}/${schedule.imagePath}`}
                      ...
                      <div>
                        <h5 className="max-w-[40vw] truncate overflow-hidden text-lg font-medium text-black">#{schedule.id}</h5>
                        <p
                          className="max-w-[40vw] truncate overflow-hidden mt-1 text-blue-600 hover:text-blue-800 hover:underline"
                          onClick={(e) => {
                            e.stopPropagation();
                            e.preventDefault();
                            window.location.href = `/spots/${schedule.spot}`;
                          }}
                        >
                          {schedule.spotName}
                        </p>
                        <p className="mt-1">{formatDateTime(schedule.startDateTime)} - {formatDateTime(schedule.endDateTime)}</p>
                        <p className="mt-1">฿{formatPrice(schedule.pricePerHour)}/hour</p>
                      </div>
                    </NavLink>
                  </td>
                  <td className="border-b py-5 pr-6 sm:pr-8 lg:pr-10">
                    <div className="flex items-center justify-end">
                      <DropdownDefault editHref={`${schedule.id}/edit`} onDelete={() => handleDelete(schedule.id)}/>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="flex items-center justify-between py-4 px-4 sm:px-6 lg:px-8">
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <p className="text-sm text-slate-500">
                Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, schedules.length)} of{' '} {schedules.length} results
              </p>
...
export default TableScheduleIndex;